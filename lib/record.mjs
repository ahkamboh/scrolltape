...
import { heroPath, getHeroTargets, getFormFields, getFormTop, getSubmitTarget, discoverNavLinks, getSectionOffsets, buildScrollStops } from "./tours.mjs";
...
async function heroInteract(page, config, w, h, doClick = true) {
...
// Type sample values into visible form fields, then click submit (guarded).
async function fillForms(page, config, w, h, pos) {
  // Real signup/contact forms usually live below the hero — scroll there first.
  const formTop = await getFormTop(page).catch(() => null);
  if (formTop != null) {
    const target = Math.max(0, Math.round(formTop - h * 0.18));
    const cur = await page.evaluate(() => window.scrollY);
    if (Math.abs(target - cur) > 40) {
      console.log("scrolltape: scrolling to the form");
      const mid = { x: w * 0.5, y: h * 0.42 };
      await Promise.all([
        smoothScroll(page, target, 750),
        moveCursor(page, pos, mid, 750),
      ]);
      pos = mid;
      await sleep(250);
    }
  }
  const fields = await getFormFields(page).catch(() => []);
  if (!fields.length) {
    console.log("scrolltape: --fill set but no form fields found");
    return pos;
  }
  console.log(`scrolltape: filling ${fields.length} form field(s)`);
  for (const f of fields) {
    await moveCursor(page, pos, { x: f.x, y: f.y }, 600);
    pos = { x: f.x, y: f.y };
    await clickAt(page, pos); // focus the field (inputs don't navigate)
    await sleep(160);
    await page.keyboard.type(f.val, { delay: 55 }); // visible character-by-character typing
    await sleep(320);
  }
  const cta = (await getSubmitTarget(page).catch(() => null)) || (await getHeroTargets(page).catch(() => []))[0];
  if (cta) {
    await moveCursor(page, pos, cta, 600);
    pos = cta;
    await sleep(260);
    console.log("scrolltape: submitting the form (guarded — won't navigate)");
...
async function tourPage(page, config, w, h, { hero }) {
  let pos;
  if (hero) {
    pos = await heroInteract(page, config, w, h, !config.fill); // hover only when we'll fill a form next
    if (config.fill) pos = await fillForms(page, config, w, h, pos);
  } else {
