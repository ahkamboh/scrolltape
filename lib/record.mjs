...
import { heroPath, getHeroTargets, getFormFields, discoverNavLinks, getSectionOffsets, buildScrollStops } from "./tours.mjs";
...
async function fillForms(page, config, w, h, pos) {
  ...
  if (!fields.length) {
    console.log("scrolltape: --fill set but no form fields in view");
    return pos;
  }
  ...
  const cta = (await getHeroTargets(page).catch(() => []))[0];
  if (cta) {
