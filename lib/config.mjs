...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--visit") args.visit = argv[++i].split(",").map((s) => s.trim()).filter(Boolean);
...
Options:
  ...
  --visit <a,b,c>        Also tour these routes/paths in the same demo (e.g. /pricing,/about)
  --follow               Auto-follow the nav links and tour those pages too
  --fill                 Type sample values into the page's form fields (signup/contact demos)
