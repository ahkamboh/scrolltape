...
// Document-space top of the first fillable field, so --fill can scroll a
// below-the-fold form (signup/contact forms usually sit under the hero) into view.
export async function getFormTop(page) {
  return page.evaluate(() => {
    const sel =
      'input:not([type=hidden]):not([type=submit]):not([type=button]):not([type=checkbox]):not([type=radio]):not([type=file]), textarea';
    for (const el of document.querySelectorAll(sel)) {
...
// The form's own submit control that's currently in view (so --fill clicks the
// real "Sign up" button, not whatever CTA happened to be in the hero).
export async function getSubmitTarget(page) {
  return page.evaluate(() => {
    const vh = window.innerHeight, vw = window.innerWidth;
    const groups = ["button[type=submit]", "input[type=submit]", "form button", "form [role=button]", "button"];
    for (const g of groups) {
      for (const el of document.querySelectorAll(g)) {
        const r = el.getBoundingClientRect();
        if (r.width < 28 || r.height < 16) continue;
        if (r.top < 0 || r.top > vh || r.left < 0 || r.right > vw + 1) continue;
